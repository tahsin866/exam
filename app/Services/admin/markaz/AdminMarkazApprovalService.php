...
class AdminMarkazApprovalService
{
    ...
    public function execute(int $agreementId, User $admin): array
    {
        try {
            ...
            // Use the status service to approve the agreement
            $success = $this->statusService->approve($agreement, 'আবেদন অনুমোদন করা হয়েছে');

            if ($success) {
                return [
                    'success' => true,
                    'message' => 'আবেদন সফলভাবে অনুমোদন করা হয়েছে!'
                ];
            } else {
                return [
                    'success' => false,
            ...
            Log::error('Markaz Approval Error: ' . $e->getMessage());

            return [
                'success' => false,
                'message' => 'অনুমোদন করতে সমস্যা হয়েছে! Error: ' . $e->getMessage()
            ];
        }
    }
}
